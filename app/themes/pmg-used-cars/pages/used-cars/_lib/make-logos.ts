// Brand-name → simple-icons slug map, mirroring the fleet convention
// (see a2zautocompletezltd/app/lib/make-logos.ts). Used to decorate the
// "Browse by make" band with brand marks; unmapped makes fall back to a monogram.
...
/** Two-letter monogram fallback for makes without a brand mark. */
export function makeMonogram(name: string): string {
  ...
  return (letters[0] ?? "") + (letters[1] ?? letters[0]?.slice(1, 2) ?? "");
}

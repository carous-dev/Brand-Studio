...
function VehicleNotFoundTemplate({
  ...
  const COLUMBUS_PHONE_DISPLAY = __contact.phoneDisplay || '+44 7738 906707';
  const COLUMBUS_PHONE_TEL = __contact.phoneTel || '+447738906707';
  const COLUMBUS_WHATSAPP_URL = __contact.whatsappUrl || 'https://wa.me/447738906707';
      ...
      <section className={styles.notFoundHero} aria-labelledby="vehicle-not-found-title">
        <div className={styles.notFoundInner}>
          ...
          <div className={styles.notFoundLinks} aria-label="Helpful shortcuts">
            ...
            <a className={styles.notFoundInlineLink} href={`tel:${COLUMBUS_PHONE_TEL}`}>
              Call {COLUMBUS_PHONE_DISPLAY}
            </a>
            <span className={styles.notFoundSeparator} aria-hidden="true">
              ·
            </span>
            <a className={styles.notFoundInlineLink} href={COLUMBUS_WHATSAPP_URL} target="_blank" rel="noreferrer">
              WhatsApp
            </a>
          </div>
        </div>
...
export function ColumbusVehicleDetailPage() {
  const __brand = useBrand();
  const __contact = getBrandContactInfo(__brand);
  const COLUMBUS_PHONE_DISPLAY = __contact.phoneDisplay || '+44 7738 906707';
  const COLUMBUS_PHONE_TEL = __contact.phoneTel || '+447738906707';
  ...
  const enquiryMileage = String(vehicle.odometer_reading_miles);
  const enquiryTransmission = vehicle.transmission_type;
  const enquiryFuelType = vehicle.fuel_type;
  const enquiryEngineSize = `${(vehicle.engine_capacity_cc / 1000).toFixed(1)}L`;
  const enquiryPriceValue = Math.round(Number(vehicle.price));
  const enquiryPriceText = formatPrice(enquiryPriceValue);
  const defaultEnquiryMessage = `Hi, I am interested in the ${enquiryVehicleName}. Please share availability and next steps.`;

  const enquiryForm = useLeadsForm<VehicleEnquiryFormValues>({
    initialValues: {
      vehicle: enquiryVehicleName,
      url: "",
      name: "",
      email: "",
      phone: "",
    ...
    buildPayload: (values, meta) => {
      const vehicleUrl = values.url || canonicalPath;
      const leadVehicleDisplay = enquiryRegistration
        ? `${enquiryVehicleName} - ${enquiryRegistration}`
        : enquiryVehicleName;
      const extraLines = [
      ...
      const payload = {
        ...
        make: vehicle.make,
        model: `${vehicle.model} ${vehicle.derivative}`.trim(),
        year: vehicle.year_of_manufacture,
        price: enquiryPriceValue,
  ...
  const setEnquiryFieldValue = enquiryForm.setFieldValue;

  useEffect(() => {
    setEnquiryFieldValue("vehicle", enquiryVehicleName);
    setEnquiryFieldValue("message", defaultEnquiryMessage);
  }, [slug, enquiryVehicleName, defaultEnquiryMessage, setEnquiryFieldValue]);
  ...
  useEffect(() => {
    if (isEnquiryOpen) {
      if (typeof window !== "undefined") {
        enquiryForm.setFieldValue("url", canonicalPath);
      }
      enquiryNameInputRef.current?.focus();
    }
  }, [canonicalPath, isEnquiryOpen]);

  useEffect(() => {
    if (enquiryForm.status === "success") {
      const timer = window.setTimeout(() => closeEnquiry(), 1400);
          ...
          <form className="vehicle-enquiry-form" onSubmit={(e) => { e.preventDefault(); void enquiryForm.submit() }}>
            ...
            <div className="vehicle-enquiry-grid vehicle-enquiry-grid-essential">
              <div className="vehicle-enquiry-field">
                ...
                {enquiryForm.errors.name ? (
                  <p className="vehicle-enquiry-field-error">{enquiryForm.errors.name}</p>
                ) : null}
              </div>
              <div className="vehicle-enquiry-field">
                ...
                {enquiryForm.errors.email ? (
                  <p className="vehicle-enquiry-field-error">{enquiryForm.errors.email}</p>
                ) : null}
              </div>
              <div className="vehicle-enquiry-field">
                ...
                {enquiryForm.errors.phone ? (
                  <p className="vehicle-enquiry-field-error">{enquiryForm.errors.phone}</p>
                ) : null}
              </div>
            </div>
              ...
              <div className="vehicle-enquiry-grid">
                <div className="vehicle-enquiry-field">
                  ...
                  <input
                    ...
                    {...enquiryForm.getFieldProps("postcode")}
                  />
                </div>
                <div className="vehicle-enquiry-field">
            ...
            <div className="vehicle-enquiry-field">
              ...
              {enquiryForm.errors.message ? (
                <p className="vehicle-enquiry-field-error">{enquiryForm.errors.message}</p>
              ) : null}
            </div>
            ...
            <div className="vehicle-enquiry-actions">
              <button type="button" className="vehicle-enquiry-btn btn-secondary" data-enquiry-close="" onClick={closeEnquiry}>
                Cancel
              </button>
              <button
                type="submit"
                className="vehicle-enquiry-btn btn-primary"
                disabled={enquiryForm.status === "submitting"}
                aria-busy={enquiryForm.status === "submitting"}
              >

...
import ChesterfieldCookieBanner from './ChesterfieldCookieBanner'
 ...
 */
export function ChesterfieldShell({ children }: { children: ReactNode }) {
  ...
  if (isSpecialArea) {
    return (
      <main id="content" role="main" className="chesterfield-main main-dashboard">
        {children}
      </main>
    )
  }
  ...
  return (
    <GarageProvider brandSlug={brand?.slug || 'default'}>
      <a href="#content" className="chesterfield-skip-link">Skip to content</a>
      <AnimateOnScroll />
      <MotionFX />
      <ScrollProgress />
      <Header />
      <main id="content" role="main" className="chesterfield-main">
        {children}
      </main>
      <Footer />
      <WhatsAppFab brand={brand} />
      <ChesterfieldCookieBanner brandSlug={brand?.slug} cookiePolicyHref="/cookie-policy" />
    </GarageProvider>
  )
}

export default ChesterfieldShell

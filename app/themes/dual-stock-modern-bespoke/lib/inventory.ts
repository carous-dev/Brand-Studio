...
export type InventoryVehicle = {
  ...
  image: string
  featured?: boolean
  // Bikes+cars discrimination (Pitfall: dual-stock theme).
  vehicleClass: VehicleClass
  engineSize?: string
  enginePower?: string
}

const BIKE_MAKES_LC = new Set([
  'harley-davidson', 'harley davidson', 'yamaha', 'honda-motorcycles', 'ducati',
  'kawasaki', 'ktm', 'suzuki-motorcycles', 'triumph', 'aprilia', 'mv agusta',
  'royal enfield', 'indian motorcycle', 'piaggio', 'vespa',
])
const BIKE_BODY_LC = new Set([
  'naked', 'sport', 'sportbike', 'cruiser', 'tourer', 'adventure', 'scooter',
  'motocross', 'enduro', 'cafe racer', 'trike', 'scrambler', 'streetfighter',
])

function detectVehicleClass(raw: any, make: string, body: string): VehicleClass {
  const declared = String(raw?.type ?? raw?.vehicle_type ?? raw?.vehicleType ?? '').toLowerCase()
  if (declared === 'bike' || declared === 'motorcycle' || declared === 'motorbike') return 'bike'
  if (declared === 'car' || declared === 'vehicle') return 'car'
  if (BIKE_MAKES_LC.has(make.toLowerCase())) return 'bike'
  if (BIKE_BODY_LC.has(body.toLowerCase())) return 'bike'
  return 'car'
}
...
export function normalizeInventoryItem(item: any): InventoryVehicle | null {
  ...
  const vehicleClass = detectVehicleClass(item, make || '', body || '')
  const engineSize = toText(vehicle.engine_size ?? vehicle.engine_capacity ?? vehicle.cc ?? item.engine_size) || undefined
  const enginePower = toText(vehicle.engine_power ?? vehicle.power ?? vehicle.horsepower ?? vehicle.bhp) || undefined
  ...
  return {
    ...
    fuel: fuel || (vehicleClass === 'bike' ? 'Petrol' : 'Petrol'),
    transmission: transmission || (vehicleClass === 'bike' ? '-' : 'Manual'),
    body: body || (vehicleClass === 'bike' ? 'Motorcycle' : 'Car'),
    make: make || 'Vehicle',
    color: color || 'Colour',
    doors: Number.isFinite(doors) && doors > 0 ? doors : (vehicleClass === 'bike' ? 0 : 4),
    location: location || '',

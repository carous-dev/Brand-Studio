...
/**
 * Convenience alias used by per-theme cards/lists — accepts a slug-or-id
 * plus a vehicle record and returns the canonical detail URL.
 */
export function buildVehicleHref(
  slugOrId: string | null | undefined,
  vehicle?: VehicleLinkInput | null,
): string {
  const merged: VehicleLinkInput = {
    slug: slugOrId || vehicle?.slug || vehicle?.derivative_slug,
    reg: vehicle?.reg ?? vehicle?.registration,
  }
  return buildVehiclePermalink(merged, '/used-cars')
}

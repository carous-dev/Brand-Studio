...
export function WarwickShell({ children }: { children: ReactNode }) {
  ...
  if (isSpecialArea) {
    return (
      <main id="content" role="main" className="warwick-main main-dashboard">
        {children}
      </main>
    )
  }
  ...
  return (
    <GarageProvider brandSlug={brand?.slug || 'default'}>
      <a href="#content" className="warwick-skip-link">Skip to content</a>
      <AnimateOnScroll />
      <MotionFX />
      <ScrollProgress />
      <PreviewBanner brand={brand} />
      <Header />
      <main id="content" role="main" className="warwick-main">
        {children}
      </main>
      <BrowseByMakes />
      <Footer />

...
/**
 * Brand-driven text helpers for the fbm-motors theme. The recipe file
 * (`recipes/text-recipe.json`) is the single source of truth for which copy
 * strings are customisable. The runtime resolves a final string per key by:
 *   1. `brand.text[<key>]` if present and non-empty
 *   2. Recipe default with token interpolation
 *
 * Tokens supported: {brandName}, {city}, {county}, {cityish}, {streetLine},
 * {postcode}, {year}, {tagline}, {namePossessive}.
 */

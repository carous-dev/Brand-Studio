...
export default function useGallery(opts: { imgs?: string[]; displayImgs?: string[]; fetchRemote?: boolean; vehicleProp?: any; remoteVehicle?: any }) {
    const { imgs = [], displayImgs = [], fetchRemote = false, vehicleProp = null, remoteVehicle = null } = opts
    ...
    const [lbImageLoading, setLbImageLoading] = useState(false)
    ...
    useEffect(() => {
        ...
        if (displayImgs && displayImgs[nextIdx]) preload(displayImgs[nextIdx])
        if (displayImgs && displayImgs[prevIdx]) preload(displayImgs[prevIdx])
    }, [index, displayImgs, len])

    function showIndex(i: number) { if (!len) return; const next = ((i % len) + len) % len; setIndex(() => next) }
    function prev() { if (!len) return; setGalleryLoading(true); setIndex(i => { const next = ((i - 1 + len) % len); return next }) }
    function next() { if (!len) return; setGalleryLoading(true); setIndex(i => { const next = ((i + 1) % len); return next }) }
    function openLightbox() { if (!len) return; setLbIndex(index); setLbOpen(true); setLbImageLoading(true) }
    ...
    useEffect(() => {
        if (lbOpen && lbIndex !== index) setLbImageLoading(true)
    }, [lbIndex, lbOpen, index])
    ...
    return {
        ...
        lbOpen, setLbOpen,
        lbIndex, setLbIndex,
        lbImageLoading, setLbImageLoading,
        mainRef, lbCloseRef,

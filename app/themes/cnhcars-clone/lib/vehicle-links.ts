import { getVehicleType, type VehicleTypeInput } from './vehicle-type'

type VehicleLinkInput = VehicleTypeInput & {
  slug?: string | null
...
export function getVehicleBasePath(
  vehicle: VehicleLinkInput | null | undefined,
  fallback: '/used-cars' | '/used-vans' = '/used-cars',
): '/used-cars' | '/used-vans' {
  if (!vehicle) return fallback
  const type = getVehicleType(vehicle)
  return type === 'van' ? '/used-vans' : '/used-cars'
}
...
export function buildVehiclePermalink(
  ...
  const permalinkSlug = reg && !baseSlug.endsWith(`-${reg.toLowerCase()}`) && baseSlug !== reg.toLowerCase()
    ? `${baseSlug}-${reg.toLowerCase()}`
    : baseSlug
  const base = getVehicleBasePath(vehicle)
  return `${base}/${encodeURIComponent(permalinkSlug)}`
}

export function splitVehiclePermalinkSlug(value: unknown): VehicleLookupCandidate {
...
export function getVehicleLookupCandidates(value: unknown): VehicleLookupCandidate[] {
  ...
  if (parsed.reg) {
    ...
    pushCandidate({ reg: normalized })
  }

  return candidates
}

export function buildVehicleStoragePathByReg(vehicle: VehicleLinkInput | null | undefined): string {
  if (!vehicle) return ''
  const regRaw = String(vehicle.reg ?? vehicle.registration ?? '').trim()
  if (!regRaw) return ''
  const reg = regRaw.replace(/[^a-zA-Z0-9]/g, '').toUpperCase()
  if (!reg) return ''
  const base = getVehicleBasePath(vehicle)
  return `${base}/${reg}`
}

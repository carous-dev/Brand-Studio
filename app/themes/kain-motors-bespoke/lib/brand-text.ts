...
/**
 * Brand-driven text helpers. The recipe file (`recipes/text-recipe.json`) is the
 * single source of truth for which copy strings are customizable — the dashboard
 * `/create` + `/update` pages read the same JSON to render form inputs, and the
 * AI brand generator reads it to know what copy to populate. The runtime resolves
 * a final string per key by:
 ...
 * expected to keep their default strings sensible when tokens are blank (e.g.
 * "Appointment-only showroom" reads fine if {cityish} is missing).
 */
...
/**
 * Legacy structured accessor — keep for components that already use the named
 * getters (Header chip, footer copyright, etc.). New code should prefer
 * `resolveText(brand, '<recipeKey>')` to opt every string into the recipe system.
 */

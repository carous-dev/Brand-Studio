/**
 * Server wrapper for the used-cars inventory list.
 *
 * Data-layer-only file (kept verbatim per SKILL pitfall #30 / inventory contract):
 * the JSX + CSS redesign lives in UsedCarsClient + page.module.css. This wrapper
 * does the per-brand server fetch, normalizes inventory, and hands off to the
 * client island. Structured as small, named helpers so the responsibilities are
 * easy to read — different style from the springalls baseline by design.
 */
...
async function resolveSearchParams(input: SearchParamsLike | Promise<SearchParamsLike>): Promise<SearchParamsLike> {
  if (input && typeof (input as any).then === 'function') {
    try {
      return await (input as Promise<SearchParamsLike>)
    } catch {
      return undefined
    }
  }
...
export async function AutoUsedCarsPage(props: UsedCarsRouteProps) {

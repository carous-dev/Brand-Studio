# showroom-shine-cars-bespoke — Component Checklist

Theme id: `showroom-shine-cars-bespoke`
Archetype: `rugged`
Generated: 2026-05-11T18:14:27.259Z
...
- [x] Advanced Filtering _(optional)_  — sticky toolbar with make / body / fuel / transmission / mileage filters + active-chip strip
- [x] Sorting Options _(optional)_  — Sort chip in the action rail (newest / price asc / price desc / lowest mileage)
- [ ] Infinite Scroll _(optional)_
- [x] Pagination _(optional)_  — numbered pagination with ellipsis, prev/next
- [x] Quick Compare _(optional)_  — Compare chip in the action rail with live count + link to `/compare`; per-card toggle in `VehicleCard`
- [x] Quick Wishlist  — Wishlist chip in the action rail with live count + link to `/wishlist`; per-card toggle in `VehicleCard`; counts come from `useGarage()` so they persist across navigation
- [x] Sticky Filters _(optional)_  — toolbar section is `position: sticky; top: 0` with sidebar filter panel sticky at 140px below
...
- [x] SEO Landing Pages _(optional)_  — Shell-level `<BrowseByMake />` band above the footer; deep links to `/used-cars?make=<Make>` per-make

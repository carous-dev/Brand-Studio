...
function buildSections(dealerName: string, address: string, email: string) {
  const contactLine = email ? `Email us at ${email} to exercise any of these rights; we respond within 30 days.` : 'Contact us to exercise any of these rights; we respond within 30 days.'

  return [
    {
      heading: '1. Who we are',
      body: [
        `${dealerName} operates${address ? ` from ${address}` : ' as a UK motor dealership'}.`,
    ...
    },
    {
      ...
      body: [
        ...
        'To match you with finance providers when you request a finance quote.',
    ...
    },
    {
      ...
      body: [
        'UK finance providers, when you request a finance quote.',
        'Vehicle data providers for stock checks.',
    ...
    },
    {
      ...
      body: [
        'Active enquiries are normally kept for 12 months. Finance applications may be retained for up to 6 years where required. Analytics data is deleted or anonymised when no longer needed.',
      ],
    },
    {
      heading: '6. Your rights',
      body: [
        `Under UK GDPR you can request access, correction, deletion, restriction or portability of your data, and withdraw consent at any time. ${contactLine}`,
      ],
    },
    {
...
export function ShowroomPrivacyPolicyPage({ brand }: ThemePageProps) {
  const dealerName = brand?.name || 'This dealership'
  const contact = getBrandContactInfo(brand)
  const sections = buildSections(dealerName, contact.showroomAddress, contact.email)
          ...
          <div className={styles.layout}>
            ...
            <div className={styles.content}>
              ...
              <p className={styles.contact}>
                Questions?{' '}
                {contact.email ? <><a href={`mailto:${contact.email}`}>Email us</a>{' '}or </> : null}

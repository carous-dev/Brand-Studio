...
/**
 * Minimal in-theme replacement for @carous/whatsapp-enquiry. Renders a
 * floating WhatsApp call-to-action linked to the brand's primary phone,
 * with an online/offline status pip driven by the brand's opening hours.
 */
...
function parseOpeningHours(hours: unknown): WorkingPeriod[] {
  const fallback: WorkingPeriod[] = (
    ['mon', 'tue', 'wed', 'thu', 'fri', 'sat'] as const
  ).map((day) => ({ day, from: '09:00', to: '18:00' }))
...
export function WhatsAppEnquiry() {
  ...
  const periods = useMemo(
    () => parseOpeningHours((brand as any)?.openingHours),
    [brand]
  )
  ...
  if (!mounted || !contact.whatsappUrl) return null

  const message = encodeURIComponent(
    `Hi ${brand?.name || 'team'}, I have a question about a vehicle I've seen on your site.`
  )
  ...
  return (
    <a
      ...
      title={statusLabel}
      className={`springalls-whatsapp-fab ${isOnline ? 'is-online' : 'is-offline'}`}
    >
      ...
      <span className="springalls-whatsapp-status" aria-hidden="true" />
      <span className="sr-only">{`Chat on WhatsApp — ${statusLabel}`}</span>
    </a>
  )
}
